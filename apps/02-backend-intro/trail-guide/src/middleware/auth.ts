...
/**
 * For JSON API mutating routes gated by admin session (as opposed to the
 * existing x-api-key + rate-limit gate on /api/trails writes, which is a
 * separate mechanism for third-party API consumers) — a redirect would
 * hand a JSON client an HTML login page, so this returns 401 JSON on
 * failure instead while enforcing the exact same credential check as
 * requireAdminAny. Not currently wired into any route — kept for parity
 * and available if an admin-gated API route is added later.
 */
