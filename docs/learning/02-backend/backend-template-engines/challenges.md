...
# Backend Template Engines - Challenges

## Code Along: Event Hub

Build a small event listing site called Event Hub. Each step builds on the previous, so work through them in order.

### Setup

Create a new Express + TypeScript project. Install the required packages:
...
Configure Nunjucks in `index.ts` as described in the Nunjucks Setup handout. Add a `.prettierrc` file with the jinja-template plugin. Install the Better Nunjucks extension in VS Code.

### Base layout

Create `views/base.html` with a `title` block and a `content` block. The base template should include a header with the site name "Event Hub", a nav with links to Home and Events, and a footer.

Create `views/index.html` that extends the base and fills the `content` block with a heading: "Welcome to Event Hub".

Add a `GET /` route that renders `index.html`.

### Events listing

Add the events below to your server and pass them to the template:
...
Create `views/events.html` that extends the base. Render each event's name, date, and location in a list. Add an `{% else %}` clause to the `for` loop that shows "No events scheduled" when the array is empty.

Add a `GET /events` route that renders `events.html` with the events data.

### Sold out badge

Add a badge to each event card that renders only when `soldOut` is `true`. Verify it works by toggling the `soldOut` value on one of the events.

### Event card macro

Extract the event card HTML into a macro called `eventCard` that accepts `name`, `date`, `location`, and an optional `soldOut` parameter defaulting to `false`. Move the macro into `views/macros/events.html` and import it in `events.html`.
...
Make sure to download the [images](#images) before starting.

**Home page**

List all posts on the home page using the [index layout](https://startbootstrap.github.io/startbootstrap-clean-blog/index.html). Each entry shows the title, teaser, author, and date. The `createdAt` field is a Unix timestamp — convert it to a human-readable date string before passing it to the template. Do not modify the JSON structure.

**Post detail page**

Render each post on its own page using the [post layout](https://startbootstrap.github.io/startbootstrap-clean-blog/post.html). The URL should include a slug derived from the post title so that each post has a unique, readable path. Display the full content and the post's image.
